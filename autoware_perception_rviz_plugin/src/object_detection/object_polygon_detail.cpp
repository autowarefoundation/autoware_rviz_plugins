  ...
  const std::vector<autoware_perception_msgs::msg::ObjectClassification>& classification)
{
  ...
  marker_ptr->mesh_resource = "package://autoware_perception_rviz_plugin/meshes/" + mesh_name + ".dae";
  ...
  for (const auto& indicator : classification) {
    ...
    if (indicator.label == 100 || indicator.label == 101 || indicator.label == 102) {
      ...
      if (indicator.label == 100) {
        marker.mesh_resource = "package://autoware_perception_rviz_plugin/meshes/" + mesh_name + "-brake.dae";
        marker.color.r = 1.0;
        marker.color.g = 0.0;
        marker.color.b = 0.0;
      } else
      {
        if (indicator.label == 101) {
          marker.mesh_resource = "package://autoware_perception_rviz_plugin/meshes/" + mesh_name + "-indicator-left.dae";
        } else if (indicator.label == 102) {
          marker.mesh_resource = "package://autoware_perception_rviz_plugin/meshes/" + mesh_name + "-indicator-right.dae";
        }
        marker.color.r = 1.0;
        marker.color.g = 1.0;
        marker.color.b = 0.0;
